    ...
    : Smoother<VectorType>(op, params)
{
  ...
  // Downcast the operator. If the operator is a CudaMatrixOperator, we need to
  // extract the diagonal and inverse it. If the operator is a
  // CudaMatrixFreeOperator, we ask the user for the inverse of the diagonal.
  auto cuda_matrix_operator =
      std::dynamic_pointer_cast<CudaMatrixOperator<VectorType> const>(
          this->_operator);
  if (cuda_matrix_operator)
  {
  ...
  else
  {
    auto cuda_matrix_free_operator =
        std::dynamic_pointer_cast<CudaMatrixFreeOperator<VectorType> const>(
            this->_operator);
    VectorType inv_diagonal = cuda_matrix_free_operator->get_diagonal_inverse();
